  ...
  return <section className="mx-auto max-w-7xl px-4 py-8 md:px-6 md:py-12">
    ...
    <div className="mt-6 rounded-3xl border border-leaf-100 bg-white p-5 shadow-soft md:p-6">
      <div className="mb-6 rounded-2xl border border-leaf-100 bg-leaf-50/40 p-4">
        ...
        <p className="mt-1 text-sm text-[#414943]">Preencha os campos para criar ou atualizar doenças do catálogo interno com padronização técnica.</p>
      </div>
      <form onSubmit={submit}>
        <div className="grid gap-4 md:grid-cols-2">{fields.map(([k, l]) => <label key={String(k)} className="text-sm font-medium text-[#414943]">{l}<textarea rows={3} required={k === "common_name"} className="mt-1 w-full rounded-2xl border border-[#e7e2d9] bg-white px-4 py-3 text-sm shadow-sm outline-none transition focus:border-leaf-400 focus:ring-2 focus:ring-leaf-100" value={String(form[k] ?? "")} onChange={(e) => setForm((c) => ({ ...c, [k]: e.target.value }))} /></label>)}
          <label className="text-sm font-medium text-[#414943]">Cultura vinculada<select className="mt-1 w-full rounded-2xl border border-[#e7e2d9] bg-white px-4 py-3 text-sm shadow-sm outline-none transition focus:border-leaf-400 focus:ring-2 focus:ring-leaf-100" value={form.crop_id ?? ""} onChange={e => setForm((c) => ({ ...c, crop_id: e.target.value }))}><option value="">Não vinculada</option>{crops.map(c => <option key={c.id} value={c.id}>{c.display_name_pt || c.name}</option>)}</select></label>
        </div>
    ...
    <div className="mt-8 rounded-3xl border border-[#e7e2d9] bg-white p-5 shadow-soft">
      <div className="flex flex-col gap-3 md:flex-row md:items-center">
        <p className="font-semibold text-[#123F2A]">Doenças cadastradas</p>
        <input className="rounded-2xl border border-[#e7e2d9] p-2 text-sm md:ml-auto md:w-72" placeholder="Buscar por nome comum ou científico" value={search} onChange={e => setSearch(e.target.value)} />
      </div>
      <div className="mt-4 space-y-2">{loadingData ? <p className="text-sm text-[#717973]">Carregando...</p> : visible.map(d => <article key={d.id} className="rounded-2xl border border-[#e7e2d9] bg-[#f8f3ea]/50 p-3"><strong>{d.common_name}</strong><p className="text-sm text-[#414943]">{d.scientific_name}</p><div className="mt-2 flex flex-wrap gap-2"><button type="button" className="rounded-full border border-leaf-200 bg-white px-3 py-1 text-xs font-medium text-leaf-700" onClick={() => { setForm({ ...empty, ...d, crop_id: d.crop_id ?? "" }); }}>Editar</button><button type="button" disabled={deletingId===d.id} className="rounded-full border border-red-200 bg-white px-3 py-1 text-xs font-medium text-red-700 disabled:opacity-60" onClick={async () => { if (!confirm("Deseja excluir este cadastro?")) return; const t = token(); if (!t) return; setDeletingId(d.id); try { const r = await fetch(`/api/specialist/diseases?id=${encodeURIComponent(d.id)}`, { method: "DELETE", headers: { Authorization: `Bearer ${t}` } }); const p = await r.json().catch(() => ({})); if (!r.ok) throw new Error(p?.error || "Falha ao excluir"); setMsg("Cadastro excluído com sucesso."); await loadData(); } catch (e) { setErr(e instanceof Error ? e.message : "Falha ao excluir"); } finally { setDeletingId(null); } }}>{deletingId===d.id ? "Excluindo..." : "Excluir"}</button></div></article>)}</div>
    </div>
  </section>;
}
