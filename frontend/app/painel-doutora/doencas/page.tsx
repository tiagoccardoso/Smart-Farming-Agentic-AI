  ...
  async function fillWithAI() {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        else if (errorCode === "ai_invalid_json" || r.status === 422) setErr("A IA respondeu, mas não foi possível estruturar os dados para o cadastro. Tente um nome mais específico.");
      ...
      const filledFieldsCount = [
        normalizedSuggestion.scientific_name,
        normalizedSuggestion.causal_agent,
        normalizedSuggestion.disease_type,
        normalizedSuggestion.symptoms,
        normalizedSuggestion.favorable_conditions,
        normalizedSuggestion.crop_stage,
        normalizedSuggestion.severity_level,
        normalizedSuggestion.management_recommendations,
        normalizedSuggestion.preventive_control,
        normalizedSuggestion.curative_control,
      ].filter((value) => value.trim().length > 0).length;

      if (filledFieldsCount < 3) {
        setErr("A IA respondeu, mas sem dados suficientes para preenchimento automático. Revise manualmente.");
        return;
      }
