...
export default function SitePageEditor() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { const s = await getCurrentAuthSession(); if (!s?.access_token) throw new Error("Sessão expirada."); const r = await fetch(`/api/admin/site-pages/${slug}`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${s.access_token}` }, body: JSON.stringify(form) }); const p = await r.json(); if (!r.ok) throw new Error(p?.error || "Não foi possível salvar as alterações. Tente novamente."); setMessage(slug === "home" ? "Página inicial atualizada com sucesso." : slug === "agricultura-organica" ? "Página de Agricultura Orgânica atualizada com sucesso." : "Página da especialista atualizada com sucesso."); } catch (e) { setError(e instanceof Error ? e.message : "Não foi possível salvar as alterações. Tente novamente."); } finally { setSaving(false); }
  }

  if (loading) return <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 md:py-14">Carregando editor...</div>;
  return <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 md:py-14"><Link href="/configuracoes" className="text-sm font-semibold text-leaf-700">← Voltar para Configurações</Link><h1 className="mt-4 text-xl font-bold text-[#123F2A] sm:text-3xl">{title}</h1>{message && <p className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">{message}</p>}{error && <p className="mt-5 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">{error}</p>}<form onSubmit={submit} className="mt-6 grid gap-5 rounded-3xl border border-leaf-100 bg-white p-6 shadow-soft"><label><span className="text-sm font-semibold">Título principal</span><input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="mt-2 w-full rounded-2xl border border-leaf-100 px-4 py-3" /></label><label><span className="text-sm font-semibold">Subtítulo principal</span><input value={form.subtitle} onChange={(e) => setForm({ ...form, subtitle: e.target.value })} className="mt-2 w-full rounded-2xl border border-leaf-100 px-4 py-3" /></label><div className="block"><span className="text-sm font-semibold">{slug === "home" ? "Enviar imagem da página inicial" : "Enviar imagem"}</span><p className="mt-1 text-xs text-slate-500">Selecione uma imagem ou tire uma foto pelo celular.</p><MobileImagePicker accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp" cameraAccept="image/*" galleryLabel="Selecionar imagem" cameraLabel="Tirar foto" galleryAriaLabel={slug === "home" ? "Selecionar imagem da página inicial" : "Selecionar imagem"} cameraAriaLabel={slug === "home" ? "Tirar foto para a página inicial" : "Tirar foto"} onGalleryChange={onUpload} onCameraChange={onUpload} className="mt-3" /></div>{preview && <div className="rounded-2xl border border-leaf-100 p-3"><img src={preview} alt={slug === "home" ? "Produção agrícola orgânica com tecnologia e orientação especializada no campo." : "Pré-visualização da foto da especialista"} className="max-h-72 w-full rounded-2xl object-cover" /></div>}
...
<button type="submit" disabled={saving||uploading} className="rounded-full bg-leaf-600 px-6 py-3 font-semibold text-white disabled:bg-slate-300">{saving ? "Salvando..." : uploading ? "Aguarde upload..." : "Salvar alterações"}</button></form></div>;
}
