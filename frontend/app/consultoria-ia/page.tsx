...
function Skeleton() {
  return <div className="animate-pulse rounded-[1.5rem] border border-[#e7e2d9] bg-white p-4 shadow-soft"><div className="h-4 w-2/3 rounded bg-[#f2ede4]" /><div className="mt-4 h-20 rounded bg-[#f2ede4]" /></div>;
}

function EmptyState({ title, description }: { title: string; description: string }) {
  return <div className="rounded-[2rem] border border-dashed border-[#e7e2d9] bg-white/80 p-8 text-center"><p className="text-lg font-bold text-[#1d1c16]">{title}</p><p className="mt-2 text-sm text-[#717973]">{description}</p></div>;
}
...
function MetricCard({ label, value, tone = "leaf" }: { label: string; value: string | number; tone?: "leaf" | "amber" | "red" | "slate" }) {
  ...
  return <div className="rounded-[1.5rem] border border-white/70 bg-white p-4 shadow-soft"><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#717973]">{label}</p><p className={`mt-3 bg-gradient-to-r ${tones[tone]} bg-clip-text text-3xl font-black text-transparent`}>{value}</p></div>;
}
...
function CaseCard({ item, active, onSelect, onAction }: { item: ListCase; active: boolean; onSelect: () => void; onAction: (action: string, item: ListCase) => void }) {
  ...
  return (
    <article className={`rounded-[1.5rem] border bg-white p-4 shadow-soft transition hover:-translate-y-0.5 ${active ? "border-leaf-400 ring-4 ring-leaf-100" : "border-[#e7e2d9]"}`}>
      <button type="button" onClick={onSelect} className="w-full text-left">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-[#717973]">{text(item.farm?.name)} · {[item.farm?.city, item.farm?.state].filter(Boolean).join("/") || "Localização não informada"}</p>
            </div>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <span className="rounded-full border border-[#e7e2d9] bg-[#f8f3ea] px-3 py-1 text-xs font-bold text-[#414943]">{attachmentCount} anexos</span>
        </div>
        <p className="mt-3 line-clamp-2 text-sm leading-6 text-[#414943]">{item.symptoms}</p>
        <p className="mt-3 text-xs font-semibold uppercase tracking-wide text-[#717973]">{formatDate(item.updated_at ?? item.created_at)}</p>
      </button>
      <div className="mt-4 grid grid-cols-2 gap-2 text-xs font-bold text-[#414943] md:grid-cols-4">
        <button onClick={() => onAction("open", item)} className="rounded-xl border border-[#e7e2d9] px-3 py-2 hover:border-leaf-300">Abrir</button>
        <button onClick={() => onAction("edit", item)} className="rounded-xl border border-[#e7e2d9] px-3 py-2 hover:border-leaf-300">Editar</button>
        <button onClick={() => onAction("ai", item)} className="rounded-xl border border-[#e7e2d9] px-3 py-2 hover:border-leaf-300">Nova IA</button>
        <button onClick={() => onAction("more", item)} className="rounded-xl border border-[#e7e2d9] px-3 py-2 hover:border-leaf-300">Ações</button>
      </div>
    </article>
  );
}
...
function DetailBlock({ label, value }: { label: string; value?: string | number | null }) {
  return <div className="rounded-2xl border border-[#e7e2d9] bg-white p-4"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#717973]">{label}</p><p className="mt-2 text-sm font-semibold leading-6 text-[#1d1c16]">{text(value)}</p></div>;
}

function ListSection({ title, items, tone = "leaf" }: { title: string; items: string[]; tone?: "leaf" | "amber" | "red" | "slate" }) {
  const tones = { leaf: "bg-leaf-50 text-leaf-700", amber: "bg-amber-50 text-amber-800", red: "bg-red-50 text-red-800", slate: "bg-[#f8f3ea] text-[#414943]" };
  return <div className={`rounded-[1.25rem] p-5 ${tones[tone]}`}><h4 className="font-bold text-slate-950">{title}</h4><ul className="mt-3 space-y-2 text-sm leading-6">{items.length ? items.map((item) => <li key={item} className="flex gap-2"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-current" />{item}</li>) : <li className="text-[#717973]">Aguardando geração de análise.</li>}</ul></div>;
}
...
function ConfidenceBar({ level }: { level?: string | null }) {
  ...
  return <div><div className="flex items-center justify-between text-xs font-black uppercase tracking-[0.16em] text-[#717973]"><span>Confiança</span><span>{confidenceLabels[level ?? ""] ?? "Não informada"}</span></div><div className="mt-2 h-3 overflow-hidden rounded-full bg-[#f2ede4]"><div className={`h-full rounded-full ${level === "high" ? "bg-leaf-600" : level === "medium" ? "bg-amber-500" : "bg-red-400"}`} style={{ width: `${percent}%` }} /></div></div>;
}

function AnalysisCard({ title, children }: { title: string; children: ReactNode }) {
  return <section className="rounded-[1.5rem] border border-[#e7e2d9] bg-white p-5 shadow-sm"><h4 className="text-sm font-black uppercase tracking-[0.16em] text-[#717973]">{title}</h4><div className="mt-3 text-sm leading-7 text-[#414943]">{children}</div></section>;
}
...
function ConsultoriaIAContent() {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#dcfce7_0,transparent_32%),linear-gradient(180deg,#f8fafc_0%,#eef7ef_100%)] px-4 py-8 text-[#1d1c16] sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[1500px] space-y-6">
        <header className="overflow-hidden rounded-[2rem] border border-white/70 bg-[#002817] p-6 text-white shadow-soft md:p-8">
        ...
        <section className="grid gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
          <aside className="space-y-4">
            <div className="rounded-[2rem] border border-white/80 bg-white/90 p-5 shadow-soft backdrop-blur">
              <div className="flex items-center justify-between gap-3"><h2 className="text-xl font-black">Casos enviados</h2><span className="rounded-full bg-[#f2ede4] px-3 py-1 text-xs font-bold text-[#414943]">Paginação · 25 por página</span></div>
              <div className="mt-4 grid gap-3">
                <input value={filters.q} onChange={(e) => setFilters((current) => ({ ...current, q: e.target.value }))} placeholder="Buscar por cultura, propriedade, cidade, sintomas..." className="rounded-2xl border border-[#e7e2d9] px-4 py-3 text-sm outline-none focus:border-leaf-400" />
                <div className="grid grid-cols-2 gap-2">
                  <select value={filters.crop} onChange={(e) => setFilters((c) => ({ ...c, crop: e.target.value }))} className="rounded-xl border border-[#e7e2d9] px-3 py-2 text-sm"><option value="">Cultura</option>{crops.map((crop) => <option key={crop}>{crop}</option>)}</select>
                  <select value={filters.status} onChange={(e) => setFilters((c) => ({ ...c, status: e.target.value }))} className="rounded-xl border border-[#e7e2d9] px-3 py-2 text-sm"><option value="">Status</option>{Object.entries(statusLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select>
                  <select value={filters.risk} onChange={(e) => setFilters((c) => ({ ...c, risk: e.target.value }))} className="rounded-xl border border-[#e7e2d9] px-3 py-2 text-sm"><option value="">Risco</option>{Object.entries(riskLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select>
                  <select value={filters.farm} onChange={(e) => setFilters((c) => ({ ...c, farm: e.target.value }))} className="rounded-xl border border-[#e7e2d9] px-3 py-2 text-sm"><option value="">Propriedade</option>{farms.map((farm) => <option key={farm}>{farm}</option>)}</select>
                  <select value={filters.period} onChange={(e) => setFilters((c) => ({ ...c, period: e.target.value }))} className="rounded-xl border border-[#e7e2d9] px-3 py-2 text-sm"><option value="">Período</option><option value="7">Últimos 7 dias</option><option value="30">Últimos 30 dias</option><option value="90">Últimos 90 dias</option></select>
                  <label className="flex items-center gap-2 rounded-xl border border-[#e7e2d9] px-3 py-2 text-xs font-bold text-[#414943]"><input type="checkbox" checked={filters.humanOnly} onChange={(e) => setFilters((c) => ({ ...c, humanOnly: e.target.checked }))} /> Só revisão humana</label>
                </div>
              </div>
            </div>
              ...
              <>
                <div className="rounded-[2rem] border border-white/80 bg-white p-6 shadow-soft">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div><p className="text-xs font-black uppercase tracking-[0.2em] text-leaf-600">Caso selecionado</p><h2 className="mt-2 text-3xl font-black text-slate-950">{selectedCase.crop}</h2><p className="mt-1 text-sm text-[#717973]">{text(selectedCase.farm?.name)} · {[selectedCase.farm?.city, selectedCase.farm?.state].filter(Boolean).join("/") || "Localização não informada"}</p></div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-3">
                    <button onClick={() => handleGenerateAnalysis()} disabled={generating} className="rounded-full bg-leaf-600 px-5 py-3 text-sm font-black text-white shadow-soft disabled:bg-[#c1c9c1]">{generating ? "Gerando análise..." : hasAnalysis ? "Gerar nova análise IA" : "Gerar análise IA"}</button>
                    <button onClick={() => requestHumanReview()} disabled={humanReviewingId === selectedId} className="rounded-full bg-[#002817] px-5 py-3 text-sm font-black text-white shadow-soft disabled:bg-[#c1c9c1]">{humanReviewingId === selectedId ? "Enviando..." : "Enviar para revisão humana"}</button>
                    <button onClick={() => setShowEdit(true)} className="rounded-full border border-[#e7e2d9] bg-white px-5 py-3 text-sm font-black text-[#414943]">Editar caso completo</button>
                ...
                <div className="grid gap-6 2xl:grid-cols-[minmax(0,1fr)_420px]">
                  <div className="space-y-6">
                    <div className="rounded-[2rem] border border-white/80 bg-white p-6 shadow-soft">
                      ...
                      <div className="mt-5 rounded-[1.5rem] bg-[#002817] p-5 text-white"><p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-300">Visão geral do caso</p><p className="mt-3 leading-7 text-slate-100">{analysis?.initialDiagnosis || selectedCase.ai_summary || "Gere uma análise para obter resumo técnico, hipóteses detalhadas, causas prováveis, riscos e recomendações iniciais seguras."}</p></div>
                      {analysis ? <div className="mt-5 grid gap-4 lg:grid-cols-3"><div className="rounded-[1.25rem] border border-[#e7e2d9] bg-[#f8f3ea] p-5"><p className="text-xs font-black uppercase tracking-[0.16em] text-[#717973]">Nível de risco</p><p className={`mt-3 inline-flex rounded-full border px-4 py-2 text-sm font-black ${levelBadgeClass(analysis.riskLevel)}`}>{riskLabels[analysis.riskLevel]}</p><p className="mt-3 text-sm leading-6 text-[#414943]">{analysis.productionImpact}</p></div><div className="rounded-[1.25rem] border border-[#e7e2d9] bg-[#f8f3ea] p-5"><ConfidenceBar level={analysis.confidenceLevel} /><p className="mt-4 text-sm leading-6 text-[#414943]">A confiança considera qualidade das imagens, dados de manejo, estágio, histórico, solo, clima e necessidade de confirmação em campo.</p></div><div className="rounded-[1.25rem] border border-amber-200 bg-amber-50 p-5"><p className="text-xs font-black uppercase tracking-[0.16em] text-amber-700">Revisão humana</p><p className="mt-3 text-sm leading-6 text-amber-900">{analysis.humanReviewReason || analysis.whenToCallHumanSpecialist}</p></div></div> : null}
                    ...
                    <div className="rounded-[2rem] border border-white/80 bg-white p-6 shadow-soft">
                      <div className="flex flex-wrap items-center justify-between gap-3"><h3 className="text-xl font-black">Interação contextual com IA</h3><div className="flex gap-2"><button onClick={() => setQuestion("Explique as recomendações em linguagem simples.")} className="rounded-full bg-[#f2ede4] px-3 py-1 text-xs font-bold">Explicar</button><button onClick={() => setQuestion("Quais informações faltam para aumentar a confiança da análise?")} className="rounded-full bg-[#f2ede4] px-3 py-1 text-xs font-bold">Perguntas</button></div></div>
                      <div className="mt-5 max-h-[440px] space-y-3 overflow-y-auto rounded-[1.5rem] bg-[#f8f3ea] p-4">
                        {chatMessages.length === 0 ? <EmptyState title="Conversa técnica vazia" description="Use a IA para complementar sintomas, interpretar recomendações e continuar a análise do caso atual." /> : chatMessages.map((message, index) => <div key={message.id ?? index} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[82%] rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === "user" ? "bg-[#002817] text-white" : "bg-white text-[#414943] shadow-sm"}`}><p className="text-xs font-black uppercase tracking-wide opacity-60">{message.role === "user" ? "Produtor" : "IA agronômica"}</p><p className="mt-1 whitespace-pre-wrap">{message.content}</p></div></div>)}
                        {chatLoading && <div className="text-sm font-bold text-leaf-700">IA processando contexto do caso...</div>}<div ref={chatEndRef} />
                      </div>
                      <form onSubmit={handleAskQuestion} className="mt-4 grid gap-3 md:grid-cols-[1fr_auto]"><textarea value={question} onChange={(e) => setQuestion(e.target.value)} rows={3} placeholder="Faça uma pergunta complementar mantendo o contexto do caso atual..." className="resize-none rounded-2xl border border-[#e7e2d9] px-4 py-3 text-sm outline-none focus:border-leaf-400" /><button disabled={chatLoading || !question.trim()} className="rounded-2xl bg-leaf-600 px-6 py-3 text-sm font-black text-white disabled:bg-[#c1c9c1]">Enviar para IA</button></form>
                    </div>
                  </div>

                  <aside className="space-y-6">
                    <div className="rounded-[2rem] border border-white/80 bg-white p-6 shadow-soft"><h3 className="text-xl font-black">Anexos e análise de solo</h3>{attachmentsState.loading ? <p className="mt-4 text-sm text-[#717973]">Carregando anexos...</p> : attachmentsState.error ? <p className="mt-4 text-sm text-red-600">{attachmentsState.error}</p> : <div className="mt-4 space-y-4">{selectedCase.images?.length ? <div className="grid grid-cols-2 gap-3">{selectedCase.images.map((image) => <div key={image.id} className="overflow-hidden rounded-2xl border border-[#e7e2d9] bg-[#f8f3ea]">{brokenAttachmentIds[image.id] ? <div className="flex aspect-square items-center justify-center p-4 text-center text-xs font-semibold text-[#414943]">Não foi possível carregar esta imagem.</div> : <img src={image.image_url} alt="Anexo do caso" className="aspect-square w-full object-cover" loading="lazy" onError={() => setBrokenAttachmentIds((current) => ({ ...current, [image.id]: true }))} />}</div>)}</div> : <p className="text-sm text-[#717973]">Nenhum anexo enviado.</p>}{selectedCase.soil_analysis_url ? <div className="rounded-2xl border border-[#e7e2d9] bg-[#f8f3ea] p-4"><p className="text-sm font-bold text-[#1d1c16]">Análise de solo</p><p className="mt-1 text-xs text-[#717973]">Tipo: documento/arquivo de solo anexado</p><a href={selectedCase.soil_analysis_url} target="_blank" rel="noreferrer" className="mt-3 inline-flex rounded-full bg-leaf-50 px-4 py-2 text-sm font-bold text-leaf-700">Visualizar ou baixar</a></div> : <p className="text-sm text-[#717973]">Nenhuma análise de solo enviada.</p>}</div>}</div>
                    <div id="case-timeline" className="rounded-[2rem] border border-white/80 bg-white p-6 shadow-soft"><h3 className="text-xl font-black">Histórico de análises</h3><div className="mt-5 space-y-4">{(activityLogs.length ? activityLogs : [{ id: "created", action: "Caso criado", created_at: selectedCase.created_at }, ...(selectedCase.ai_summary ? [{ id: "ai", action: "IA analisou", created_at: selectedCase.created_at }] : []), ...(selectedCase.human_review_requested ? [{ id: "human", action: "Revisão humana solicitada", created_at: selectedCase.created_at }] : [])]).map((log) => <div key={log.id} className="flex gap-3"><span className="mt-1 h-3 w-3 rounded-full bg-leaf-600 ring-4 ring-leaf-100" /><div><p className="font-bold text-[#1d1c16]">{log.action}</p><p className="text-xs text-[#717973]">{formatDate(log.created_at)}</p></div></div>)}</div></div>
                    <div className="rounded-[2rem] border border-amber-200 bg-amber-50 p-6 shadow-soft"><h3 className="text-xl font-black text-amber-950">Revisão humana rápida</h3><p className="mt-2 text-sm leading-6 text-amber-900">Encaminhe casos críticos para validação por especialista. Planos sem permissão veem a oferta comercial automaticamente.</p><button onClick={() => requestHumanReview()} disabled={humanReviewingId === selectedId} className="mt-4 w-full rounded-full bg-amber-600 px-5 py-3 text-sm font-black text-white disabled:bg-[#c1c9c1]">{humanReviewingId === selectedId ? "Enviando..." : "Enviar para revisão humana"}</button></div>
                  </aside>
                </div>
                <SafetyDisclaimer />
              </>
      ...
      {showEdit && selectedCase && <div className="fixed inset-0 z-50 overflow-y-auto bg-[#002817]/60 p-4 backdrop-blur"><div className="mx-auto my-8 max-w-4xl rounded-[2rem] bg-white p-6 shadow-soft"><div className="flex items-start justify-between gap-4"><div><h2 className="text-2xl font-black">Editar caso completo</h2><p className="mt-1 text-sm text-[#717973]">Atualize cultura, propriedade, área, solo, estágio, sintomas, histórico, imagens e análise de solo.</p></div><button onClick={() => setShowEdit(false)} className="rounded-full bg-[#f2ede4] px-4 py-2 font-bold">Fechar</button></div><div className="mt-6 grid gap-4 md:grid-cols-2">{([ ["crop","Cultura"], ["farmName","Propriedade"], ["city","Cidade"], ["state","Estado"], ["areaHectares","Área (ha)"], ["soilType","Tipo de solo"], ["growthStage","Estágio da cultura"] ] as [keyof EditForm,string][]).map(([key,label]) => <label key={key} className="text-sm font-bold text-[#414943]">{label}<input value={editForm[key]} onChange={(e) => setEditForm((c) => ({ ...c, [key]: e.target.value }))} className="mt-2 w-full rounded-2xl border border-[#e7e2d9] px-4 py-3 font-normal outline-none focus:border-leaf-400" /></label>)}<label className="md:col-span-2 text-sm font-bold text-[#414943]">Sintomas<textarea value={editForm.symptoms} onChange={(e) => setEditForm((c) => ({ ...c, symptoms: e.target.value }))} rows={4} className="mt-2 w-full rounded-2xl border border-[#e7e2d9] px-4 py-3 font-normal outline-none focus:border-leaf-400" /></label><label className="md:col-span-2 text-sm font-bold text-[#414943]">Histórico<textarea value={editForm.managementHistory} onChange={(e) => setEditForm((c) => ({ ...c, managementHistory: e.target.value }))} rows={4} className="mt-2 w-full rounded-2xl border border-[#e7e2d9] px-4 py-3 font-normal outline-none focus:border-leaf-400" /></label><label className="rounded-2xl border border-dashed border-[#c1c9c1] p-4 text-sm font-bold text-[#414943]">Adicionar novas imagens<input type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={(e: ChangeEvent<HTMLInputElement>) => setNewImages(Array.from(e.target.files ?? []))} className="mt-3 block w-full text-sm font-normal" /></label><label className="rounded-2xl border border-dashed border-[#c1c9c1] p-4 text-sm font-bold text-[#414943]">Substituir/adicionar análise de solo<input type="file" accept="application/pdf,image/jpeg,image/png" onChange={(e) => setSoilFile(e.target.files?.[0] ?? null)} className="mt-3 block w-full text-sm font-normal" /></label></div><div className="mt-6 flex flex-wrap justify-end gap-3"><button onClick={() => handleSaveCase(false)} disabled={savingEdit} className="rounded-full border border-[#e7e2d9] px-5 py-3 text-sm font-black">Salvar rascunho</button><button onClick={() => hasAnalysis && !window.confirm("Deseja gerar uma nova análise com base nas alterações?") ? handleSaveCase(false) : handleSaveCase(true)} disabled={savingEdit} className="rounded-full bg-leaf-600 px-5 py-3 text-sm font-black text-white">Salvar e reenviar para IA</button></div></div></div>}
      {showDelete && <div className="fixed inset-0 z-50 grid place-items-center bg-[#002817]/60 p-4 backdrop-blur"><div className="max-w-lg rounded-[2rem] bg-white p-6 shadow-soft"><h2 className="text-2xl font-black">Confirmar exclusão segura</h2><p className="mt-3 text-sm leading-6 text-[#414943]">Esta ação não poderá ser desfeita. O caso, respostas, registros de revisão e arquivos enviados serão excluídos permanentemente. Digite <strong>EXCLUIR</strong> para confirmar.</p><label className="mt-5 block text-sm font-bold text-[#414943]">Palavra de confirmação<input value={deleteConfirmation} onChange={(event) => { setDeleteConfirmation(event.target.value); setDeleteFeedback(event.target.value && event.target.value.trim().toUpperCase() !== "EXCLUIR" ? "A palavra ainda não confere com EXCLUIR." : null); }} disabled={deletingCase} className="mt-2 w-full rounded-2xl border border-[#e7e2d9] px-4 py-3 font-normal outline-none focus:border-red-300" placeholder="EXCLUIR" /></label>{deleteFeedback && <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-sm font-semibold text-amber-800">{deleteFeedback}</p>}<div className="mt-6 flex justify-end gap-3"><button onClick={() => { if (!deletingCase) { setShowDelete(false); setDeleteConfirmation(""); setDeleteFeedback(null); } }} disabled={deletingCase} className="rounded-full border border-[#e7e2d9] px-5 py-3 text-sm font-black disabled:opacity-60">Cancelar</button><button onClick={handleDelete} disabled={!isDeletingEnabled || deletingCase} className="rounded-full bg-red-600 px-5 py-3 text-sm font-black text-white disabled:bg-[#c1c9c1]">{deletingCase ? "Excluindo..." : "Excluir caso"}</button></div></div></div>}
      {showUpsell && <div className="fixed inset-0 z-50 grid place-items-center bg-[#002817]/60 p-4 backdrop-blur"><div className="max-w-2xl rounded-[2rem] bg-white p-6 shadow-soft"><h2 className="text-2xl font-black">Revisão humana disponível no Premium</h2><p className="mt-3 text-sm leading-6 text-[#414943]">Seu plano atual não possui revisão humana inclusa. Escolha uma opção para encaminhar o caso a um especialista.</p><div className="mt-5 grid gap-4 md:grid-cols-2"><div className="rounded-2xl border border-amber-200 bg-amber-50 p-5"><p className="font-black">Revisão avulsa</p><p className="mt-2 text-3xl font-black">R$ 197</p><Link href={`/revisao-humana?caseId=${selectedId}`} className="mt-4 inline-flex rounded-full bg-amber-600 px-5 py-3 text-sm font-black text-white">Contratar avulsa</Link></div><div className="rounded-2xl border border-leaf-200 bg-leaf-50 p-5"><p className="font-black">Premium mensal</p><p className="mt-2 text-3xl font-black">R$ 397</p><Link href="/planos" className="mt-4 inline-flex rounded-full bg-leaf-600 px-5 py-3 text-sm font-black text-white">Ver Premium</Link></div></div><button onClick={() => setShowUpsell(false)} className="mt-5 rounded-full border border-[#e7e2d9] px-5 py-3 text-sm font-black">Fechar</button></div></div>}
      {showActions && <div className="fixed inset-0 z-50 grid place-items-center bg-[#002817]/60 p-4 backdrop-blur"><div className="w-full max-w-md rounded-[2rem] bg-white p-6 shadow-soft"><h2 className="text-xl font-black">Ações do caso</h2><div className="mt-4 grid gap-2">{[["open","Abrir caso"],["edit","Editar caso completo"],["delete","Excluir caso"],["ai","Gerar nova análise IA"],["human","Solicitar revisão humana"],["report","Abrir relatório PDF"],["duplicate","Duplicar caso"],["history","Ver histórico do caso"]].map(([action,label]) => <button key={action} onClick={() => handleAction(action, showActions)} disabled={action === "human" && humanReviewingId === showActions.id} className="rounded-2xl border border-[#e7e2d9] px-4 py-3 text-left text-sm font-bold hover:border-leaf-300 disabled:opacity-60">{action === "human" && humanReviewingId === showActions.id ? "Enviando para revisão..." : label}</button>)}</div><button onClick={() => setShowActions(null)} className="mt-4 rounded-full bg-[#f2ede4] px-5 py-3 text-sm font-black">Fechar</button></div></div>}
    </main>
  );
}

export default function ConsultoriaIAPage() {
  return <Suspense fallback={<div className="mx-auto max-w-6xl px-6 py-14 text-sm text-[#414943]">Carregando consultoria...</div>}><ConsultoriaIAContent /></Suspense>;
}
