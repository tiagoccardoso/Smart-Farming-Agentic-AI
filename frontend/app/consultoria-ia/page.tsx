...
type UpsellPrices = { reviewPriceCents: number | null; premiumPriceCents: number | null };
...
function formatCents(value: number | null) {
  return value === null ? "Consulte o valor atualizado" : new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(value / 100);
}
...
function ConsultoriaIAContent() {
  ...
  const [upsellPrices, setUpsellPrices] = useState<UpsellPrices>({ reviewPriceCents: null, premiumPriceCents: null });
  ...
  useEffect(() => {
    fetch("/api/plans", { cache: "no-store" }).then((response) => response.ok ? response.json() : null).then((payload) => {
      const review = payload?.services?.find((service: { service_type?: string }) => service.service_type === "human_case_review");
      const premium = payload?.plans?.find((item: { slug?: string }) => item.slug === "ia-revisao-humana");
      setUpsellPrices({ reviewPriceCents: typeof review?.price_cents === "number" ? review.price_cents : null, premiumPriceCents: typeof premium?.price_cents === "number" ? premium.price_cents : null });
    }).catch(() => null);
  }, []);
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#dcfce7_0,transparent_32%),linear-gradient(180deg,#f8fafc_0%,#eef7ef_100%)] px-3 py-6 text-slate-900 sm:px-6 sm:py-8 lg:px-8">
      ...
      {showUpsell && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-3 backdrop-blur sm:p-4"><div className="w-full max-w-2xl rounded-[2rem] bg-white p-4 shadow-soft sm:p-6"><h2 className="text-2xl font-black">Revisão humana disponível no Premium</h2><p className="mt-3 text-sm leading-6 text-slate-600">Seu plano atual não possui revisão humana inclusa. Escolha uma opção para encaminhar o caso a um especialista.</p><div className="mt-5 grid gap-4 md:grid-cols-2"><div className="rounded-2xl border border-amber-200 bg-amber-50 p-5"><p className="font-black">Revisão avulsa</p><p className="mt-2 text-2xl font-black sm:text-3xl">{formatCents(upsellPrices.reviewPriceCents)}</p><Link href={`/revisao-humana?caseId=${selectedId}`} className="mt-4 inline-flex rounded-full bg-amber-600 px-5 py-3 text-sm font-black text-white">Contratar avulsa</Link></div><div className="rounded-2xl border border-leaf-200 bg-leaf-50 p-5"><p className="font-black">Premium mensal</p><p className="mt-2 text-2xl font-black sm:text-3xl">{formatCents(upsellPrices.premiumPriceCents)}</p><Link href="/planos" className="mt-4 inline-flex rounded-full bg-leaf-600 px-5 py-3 text-sm font-black text-white">Ver Premium</Link></div></div><button onClick={() => setShowUpsell(false)} className="mt-5 rounded-full border border-slate-200 px-5 py-3 text-sm font-black">Fechar</button></div></div>}
