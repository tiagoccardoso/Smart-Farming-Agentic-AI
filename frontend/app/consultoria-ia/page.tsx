...
function ConsultoriaIAContent() {
  ...
  async function requestHumanReview(caseId = selectedId) {
    ...
    try {
      ...
      if (response.status === 402) {
        ...
        setShowUpsell(true);
        showToast("error", payload?.error || "Seu plano atual não permite solicitar parecer agronômico humano neste momento.");
        return;
      }
      ...
      showToast("success", payload?.humanOpinion?.balanceLabel ? `Caso enviado para parecer agronômico. ${payload.humanOpinion.balanceLabel}.` : "Caso enviado para parecer agronômico da especialista.");
      await loadCases(caseId);
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#dcfce7_0,transparent_32%),linear-gradient(180deg,#f8fafc_0%,#eef7ef_100%)] px-3 py-6 text-slate-900 sm:px-6 sm:py-8 lg:px-8">
        ...
        <header className="overflow-hidden rounded-[2rem] bg-gradient-to-br from-leaf-900 via-leaf-800 to-emerald-700 p-5 text-white shadow-soft sm:p-6 md:p-8">
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
            ...
            <div className="grid w-full gap-3 sm:w-auto sm:grid-cols-2 lg:flex lg:justify-end">
              <Link href="/enviar-caso?destino=consultoria-ia" className="rounded-full bg-white px-5 py-3 text-center text-sm font-black text-leaf-800 shadow-soft transition hover:-translate-y-0.5 hover:bg-leaf-50">Novo Caso</Link>
              ...
              <>
                <div className="rounded-[2rem] border border-white/80 bg-white p-6 shadow-soft">
                  ...
                  <div className="mt-5 grid gap-3 sm:flex sm:flex-wrap">
                    ...
                    <button onClick={() => requestHumanReview()} disabled={humanReviewingId === selectedId} className="w-full rounded-full bg-leaf-700 px-5 py-3 text-sm font-black text-white shadow-soft hover:bg-leaf-800 disabled:bg-slate-300 sm:w-auto">{humanReviewingId === selectedId ? "Enviando..." : "Solicitar parecer agronômico"}</button>
                ...
                <div className="grid gap-6 2xl:grid-cols-[minmax(0,1fr)_420px]">
                  ...
                  <aside className="space-y-6">
                    ...
                    <div className="rounded-[2rem] border border-amber-200 bg-amber-50 p-6 shadow-soft"><h3 className="text-xl font-black text-amber-950">Parecer agronômico humano</h3><p className="mt-2 text-sm leading-6 text-amber-900">Encaminhe casos críticos para a especialista. Cada solicitação consome 1 parecer do benefício mensal do seu plano.</p><button onClick={() => requestHumanReview()} disabled={humanReviewingId === selectedId} className="mt-4 w-full rounded-full bg-amber-600 px-5 py-3 text-sm font-black text-white disabled:bg-slate-300">{humanReviewingId === selectedId ? "Enviando..." : "Solicitar parecer agronômico"}</button></div>
                  </aside>
                </div>
                <SafetyDisclaimer />
              </>
      ...
      {showUpsell && <div role="dialog" aria-modal="true" aria-labelledby="upsell-parecer-titulo" className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-3 backdrop-blur sm:p-4"><div className="w-full max-w-xl rounded-[2rem] bg-white p-4 shadow-soft sm:p-6"><h2 id="upsell-parecer-titulo" className="text-2xl font-black">Parecer agronômico humano</h2><p className="mt-3 text-sm leading-6 text-slate-600">{upsellMessage}</p><div className="mt-5 grid gap-3 sm:flex sm:flex-wrap"><Link href="/planos" className="rounded-full bg-leaf-600 px-5 py-3 text-center text-sm font-black text-white">Ver planos</Link><button onClick={() => setShowUpsell(false)} className="rounded-full border border-slate-200 px-5 py-3 text-sm font-black">Fechar</button></div></div></div>}
