  ...
  return (
    <div className="bg-[#fef9f0] px-6 py-14">
      <div className="mx-auto max-w-6xl">
        <section className="grid gap-8 rounded-[2rem] border border-[#123F2A]/15 bg-gradient-to-br from-white via-[#F8FCF7] to-[#EEF6ED] p-8 shadow-soft md:grid-cols-[0.8fr_1.2fr] md:p-12 md:items-center">
          ...
          <div>
            ...
            <p className="mt-5 max-w-4xl leading-7 text-[#414943]">{content.summary}</p>
          </div>
        </section>

        <section className="mt-10 grid gap-6 md:grid-cols-2">
          <article className="rounded-3xl border border-[#123F2A]/10 bg-white p-7 shadow-soft">
            <h2 className="text-2xl font-bold text-[#123F2A]">Formação acadêmica</h2>
            <ul className="mt-4 space-y-3 text-[#414943]">{education.map((item) => <li key={item}>• {item}</li>)}</ul>
          </article>
          <article className="rounded-3xl border border-[#123F2A]/10 bg-white p-7 shadow-soft">
            <h2 className="text-2xl font-bold text-[#123F2A]">Competências</h2>
            <ul className="mt-4 grid gap-2 text-[#414943]">{competencies.map((item) => <li key={item}>• {item}</li>)}</ul>
          </article>
        </section>

        <section className="mt-6 rounded-3xl border border-[#123F2A]/10 bg-white p-7 shadow-soft">
          <h2 className="text-2xl font-bold text-[#123F2A]">Experiências relevantes</h2>
          <ul className="mt-4 grid gap-3 text-[#414943]">{experiences.map((item) => <li key={item}>• {item}</li>)}</ul>
        </section>

        <section className="mt-6 rounded-3xl border border-[#2E7D32]/20 bg-[#123F2A] p-7 text-white shadow-soft">
