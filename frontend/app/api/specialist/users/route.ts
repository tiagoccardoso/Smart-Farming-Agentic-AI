...
function getSupabaseAdminConfig() {
  ...
  if (!serviceRoleKey) {
    throw new Error(
      "Configure SUPABASE_SERVICE_ROLE_KEY para gerenciar usuários.",
    );
  }
  ...
  if (!profile || !["specialist", "admin"].includes(profile.role)) {
    return {
      error: NextResponse.json(
        {
          error:
            "Acesso negado. Apenas especialistas e administradores podem gerenciar usuários.",
        },
        { status: 403 },
      ),
    };
...
function buildProfileQuery(searchParams: URLSearchParams) {
  const filters = [
    "select=id,full_name,role,phone,created_at,status,unlimited_access",
  ...
  try {
    ...
    if (!token) {
      return NextResponse.json(
        { error: "Faça login para gerenciar usuários." },
        { status: 401 },
      );
    }
    ...
    const [
      ...
      supabaseAdminRequest<AuthAdminUsersResponse>(
        "/auth/v1/admin/users?page=1&per_page=1000",
        { method: "GET" },
      ).catch(() => ({ users: [] })),
      supabaseAdminRequest<SubscriptionRow[]>(
        "/rest/v1/subscriptions?select=id,user_id,status,current_period_end,created_at,plans(id,slug,name)&order=created_at.desc",
        { method: "GET" },
      ),
      supabaseAdminRequest<FarmRow[]>(
        "/rest/v1/farms?select=id,user_id,name,city,state,area_hectares,soil_type,created_at&order=created_at.desc",
        { method: "GET" },
      ),
      supabaseAdminRequest<CaseRow[]>(
        "/rest/v1/agronomic_cases?select=id,user_id,crop,status,risk_level,human_review_requested,human_review_status,created_at&order=created_at.desc&limit=1000",
        { method: "GET" },
      ),
      supabaseAdminRequest<ReportRow[]>(
        "/rest/v1/reports?select=id,user_id,case_id,report_type,report_url,created_at&order=created_at.desc&limit=1000",
        { method: "GET" },
      ),
      supabaseAdminRequest<PaymentRow[]>(
        "/rest/v1/one_time_orders?select=id,user_id,case_id,service_type,price_cents,payment_status,created_at&order=created_at.desc&limit=1000",
        { method: "GET" },
      ),
      supabaseAdminRequest<UsageRow[]>(
        "/rest/v1/usage_events?select=id,user_id,event_type,count,period_start,period_end,created_at&order=created_at.desc&limit=1000",
        { method: "GET" },
      ),
      supabaseAdminRequest<CountRow[]>(
        "/rest/v1/agronomic_cases?select=user_id,count",
        { method: "GET", headers: { Prefer: "count=exact" } },
      ).catch(() => []),
      supabaseAdminRequest<CountRow[]>(
        "/rest/v1/one_time_orders?service_type=ilike.*review*&payment_status=eq.paid&select=user_id,count",
        { method: "GET", headers: { Prefer: "count=exact" } },
      ).catch(() => []),
    ]);

    const authUsersById = new Map(
      (authUsersResponse.users ?? []).map((user) => [user.id, user]),
    );
    ...
    const users = profiles
      .map((profile) => {
        ...
        const planName =
          ...
          (currentSubscription ? "Plano sem nome" : null);
        const planSlug = currentSubscription?.plans?.slug ?? null;
        const userCases = casesByUser[profile.id] ?? [];
        const paidReviews =
          paymentsByUser[profile.id]?.filter(
            (payment) =>
              payment.payment_status === "paid" &&
              (payment.service_type ?? "").toLowerCase().includes("review"),
          ) ?? [];
        ...
        return {
          ...
          human_reviews_count:
            reviewsCountByUser[profile.id] ?? paidReviews.length,
          details: {
      ...
      .filter(
        (user) =>
          !plan ||
          user.current_plan?.slug === plan ||
          user.current_plan?.name === plan ||
          (!user.current_plan && plan === "sem-plano"),
      );
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!token) {
      return NextResponse.json(
        { error: "Faça login para alterar usuários." },
        { status: 401 },
      );
    }
    ...
    const payload = (await request
      .json()
      .catch(() => null)) as UserMutationPayload | null;

    if (!payload) {
      return NextResponse.json(
        { error: "Informe os dados da alteração." },
        { status: 400 },
      );
    }
    ...
    if (!targetUserId) {
      return NextResponse.json(
        { error: "Informe o usuário que será alterado." },
        { status: 400 },
      );
    }
    ...
    if (auth.profile.role !== "admin" && target.role === "admin") {
      return NextResponse.json(
        { error: "Especialistas não podem alterar administradores." },
        { status: 403 },
      );
    }
    ...
    const updates: Partial<
      Pick<
        ProfileRow,
        "full_name" | "phone" | "status" | "unlimited_access" | "role"
      >
    > = {};
    ...
    if (payload.role !== undefined) {
      ...
      if (auth.profile.role !== "admin" && payload.role === "admin") {
        return NextResponse.json(
          { error: "Especialistas não podem promover usuários para admin." },
          { status: 403 },
        );
      }
      if (auth.profile.role !== "admin" && target.role === "admin") {
        return NextResponse.json(
          { error: "Apenas admin pode alterar outro admin." },
          { status: 403 },
        );
      }
    ...
    const wantsPlanChange = "current_plan_slug" in payload;
    if (Object.keys(updates).length === 0 && !email && !wantsPlanChange) {
      return NextResponse.json(
    ...
    if (target.id === auth.user.id) {
      ...
      if (
        nextStatus !== "active" ||
        !["specialist", "admin"].includes(nextRole)
      ) {
        return NextResponse.json(
    ...
    if (wantsPlanChange) {
      const desiredSlug = payload.current_plan_slug?.trim() || null;
      if (desiredSlug) {
        const plans = await supabaseAdminRequest<PlanCatalogRow[]>(
          `/rest/v1/plans?slug=eq.${encodeURIComponent(desiredSlug)}&select=id,slug,name&limit=1`,
          { method: "GET" },
        );
        const plan = plans[0];
        if (!plan) {
        ...
        const currentSubscriptions = await supabaseAdminRequest<
          SubscriptionRow[]
        >(
          `/rest/v1/subscriptions?user_id=eq.${encodeURIComponent(targetUserId)}&select=id,user_id,status,current_period_end,created_at,plans(id,slug,name)&order=created_at.desc&limit=1`,
          { method: "GET" },
        );
        const current = currentSubscriptions[0];
        if (current) {
          await supabaseAdminRequest(
            `/rest/v1/subscriptions?id=eq.${encodeURIComponent(current.id)}`,
            {
              method: "PATCH",
          ...
          await supabaseAdminRequest("/rest/v1/subscriptions", {
            method: "POST",
            headers: { Prefer: "return=minimal" },
            body: JSON.stringify({
    ...
    const updatedRows = Object.keys(updates).length
      ? await supabaseAdminRequest<ProfileRow[]>(
