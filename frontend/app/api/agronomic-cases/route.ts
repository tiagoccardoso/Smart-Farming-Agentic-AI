...
function validateUpload(file: File, allowedTypes: string[], allowedExtensions: string[], label: string) {
  const extension = getFileExtension(file.name);

  if (!allowedTypes.includes(file.type) || !allowedExtensions.includes(extension)) {
    throw new Error(`${label}: envie apenas arquivos nos formatos permitidos.`);
  }

  if (file.size > MAX_FILE_SIZE_BYTES) {
    throw new Error(`${label}: o arquivo ${file.name} ultrapassa o limite de 10MB.`);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    photoFiles.forEach((photo) => validateUpload(photo, ACCEPTED_IMAGE_TYPES, ACCEPTED_IMAGE_EXTENSIONS, "Fotos"));

    if (isFile(soilAnalysis)) {
      validateUpload(soilAnalysis, ACCEPTED_SOIL_ANALYSIS_TYPES, ACCEPTED_SOIL_ANALYSIS_EXTENSIONS, "Análise de solo");
    }
