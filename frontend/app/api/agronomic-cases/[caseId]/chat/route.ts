...
async function generateAssistantTurn(
  ...
  const remainingPendingQuestions = pendingQuestions
  ...
  const analysisQuestion = [
    ...
    remainingPendingQuestions.length > 0
      ? `Próxima pergunta pendente da fila: ${remainingPendingQuestions[0].question}`
      : "Todas as perguntas pendentes registradas para este caso foram respondidas. Não informe que ainda faltam esses dados da fila; se ainda houver incerteza, explique como limitação da triagem e recomende revisão humana quando houver risco.",
  ...
  const analysisForResponse = {
    ...analysis,
    missingQuestions: remainingPendingQuestions.map(
  ...
  const nextQuestion =
    pendingState.next ?? remainingPendingQuestions[0] ?? null;

  const assistantText = nextQuestion
    ? `${analysisForResponse.conversationalAnswer?.trim() || "Entendi. Vou atualizar o contexto e avançar para a próxima pergunta pendente."}\n\n${nextQuestion.question}`
    : analysisForResponse.conversationalAnswer ||
      "Informação registrada no caso. Todas as perguntas pendentes da fila foram respondidas. Continue enviando detalhes, fotos ou áudio se os sintomas evoluírem; para decisões de manejo com risco, solicite revisão humana.";
