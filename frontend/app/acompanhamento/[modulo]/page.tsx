...
function isImageField(field: Field) {
  return /(foto|fotos|imagem|imagens|photo|image)/i.test(`${field.key} ${field.label}`);
}

function AcompanhamentoFileInput({ field, value, onChange }: { field: Field; value?: FormValue; onChange: (file: File | null) => void }) {
  const imageOnly = isImageField(field);
  const selectedLabel = value instanceof File ? value.name : typeof value === "string" && value ? "Arquivo já salvo" : "";

  return (
    <div className="mt-1 rounded-2xl border border-dashed border-leaf-200 bg-white p-3">
      <MobileImagePicker
        accept={imageOnly ? "image/*,.jpg,.jpeg,.png,.webp" : "application/pdf,image/*,.pdf,.jpg,.jpeg,.png,.webp"}
        cameraAccept="image/*"
        galleryLabel={imageOnly ? "Selecionar imagem" : "Selecionar arquivo"}
        cameraLabel="Tirar foto"
        galleryAriaLabel={`Selecionar ${field.label}`}
        cameraAriaLabel={`Tirar foto para ${field.label}`}
        onGalleryChange={(event) => onChange(event.target.files?.[0] ?? null)}
        onCameraChange={(event) => onChange(event.target.files?.[0] ?? null)}
      />
      <p className="mt-2 text-xs font-normal text-slate-500">
        {imageOnly ? "Selecione uma imagem da galeria ou use a câmera do celular." : "Selecione um arquivo ou tire uma foto quando o anexo for uma imagem."}
      </p>
      {selectedLabel && (
        <div className="mt-3 rounded-xl bg-leaf-50 p-3 text-xs font-semibold text-slate-700">
          <span className="break-all">Selecionado: {selectedLabel}</span>
          <button type="button" onClick={() => onChange(null)} className="ml-2 font-black text-red-700 underline">Remover</button>
        </div>
      )}
    </div>
  );
...
export default function Page() {
  ...
  const [form, setForm] = useState<Record<string, FormValue>>({ id: "" });
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    for (const f of fields) { const value = form[f.key]; const empty = value instanceof File ? value.size <= 0 : !String(value ?? "").trim(); if (f.required && empty) return setMessage({ type: "error", text: `Campo obrigatório: ${f.label}.` }); }
    ...
    try {
      ...
      const token = getStoredSupabaseAccessToken();
      const amount = typeof form.custo_safra === "string" && form.custo_safra ? Number(form.custo_safra.replace(/\./g, "").replace(",", ".")) : null;
      const isProperty = modulo === "cadastro-propriedade";
      const res = await fetch(isProperty ? (form.id ? `/api/acompanhamento/properties/${form.id}` : "/api/acompanhamento/properties") : (form.id ? `/api/acompanhamento/${modulo}/${form.id}` : `/api/acompanhamento/${modulo}`), { method: form.id ? "PUT" : "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify(isProperty ? { name: form.nome_propriedade, owner_name: form.proprietario, location_gps: form.localizacao_gps || null, total_area_ha: form.area_total ? Number(String(form.area_total).replace(",", ".")) : null, sectors: form.talhoes_setores ? String(form.talhoes_setores).split(",").map((x) => x.trim()).filter(Boolean) : [], soil_type: form.tipo_solo || null, altitude_m: form.altitude ? Number(form.altitude) : null, area_history: form.historico_area || null, photo_urls: typeof data.fotos_propriedade === "string" && data.fotos_propriedade ? [data.fotos_propriedade] : [] } : { title: form.nome_propriedade || form.cultura_atual || form.safra || "Registro", property_id: form.property_id || null, amount, data }) });
      const payload = await res.json();
  ...
  return <section className="mx-auto max-w-7xl px-4 py-8 md:px-6"><Link href="/acompanhamento" className="text-sm font-semibold text-leaf-700">← Voltar</Link><div className="mt-4 rounded-3xl border border-leaf-100 bg-white p-5 shadow-soft md:p-8"><h1 className="text-xl font-bold text-[#123F2A] sm:text-2xl">{moduleData.title}</h1><p className="mt-1 text-sm text-slate-500">Preencha os campos abaixo para criar ou atualizar o cadastro.</p>{message && <p className={`mt-4 rounded-2xl border p-3 text-sm ${message.type === "success" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-red-200 bg-red-50 text-red-800"}`}>{message.text}</p>}<form onSubmit={submit} className="mt-5 grid gap-4 md:grid-cols-2">{fields.map((f) => { const isCropField = cropFieldKeys.has(f.key); return <label key={f.key} className="text-sm font-medium text-slate-700">{f.label}{f.required ? " *" : ""}{f.type === "textarea" ? <textarea className="mt-1 w-full rounded-2xl border border-leaf-100 bg-white px-4 py-3 text-sm" value={String(form[f.key] ?? "")} onChange={(e) => setForm((x) => ({ ...x, [f.key]: e.target.value }))} /> : f.type === "select" ? <select className="mt-1 w-full rounded-2xl border border-leaf-100 bg-white px-4 py-3 text-sm" value={String(form[f.key] ?? "")} onChange={(e) => setForm((x) => ({ ...x, [f.key]: e.target.value }))}><option value="">{loadingCrops && isCropField ? "Carregando culturas..." : "Selecione"}</option>{isCropField ? crops.map((c) => { const cropName = c.display_name_pt || c.name || ""; return <option key={c.id} value={cropName}>{cropName}</option>; }) : f.key === "doenca_praga" ? diseases.map((d)=><option key={d.id} value={d.common_name}>{d.common_name}</option>) : f.options ? f.options.map((o) => <option key={o} value={o}>{o}</option>) : properties.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select> : f.type === "file" ? <AcompanhamentoFileInput field={f} value={form[f.key]} onChange={(file) => setForm((x) => ({ ...x, [f.key]: file ?? "" }))} /> : <input className="mt-1 w-full rounded-2xl border border-leaf-100 bg-white px-4 py-3 text-sm" type={f.type === "date" ? "date" : "text"} placeholder={f.type === "currency" ? "R$ 1.234,56" : undefined} value={String(form[f.key] ?? "")} onChange={(e) => setForm((x) => ({ ...x, [f.key]: e.target.value }))} />}{isCropField && !loadingCrops && crops.length === 0 ? <span className="mt-1 block text-xs text-slate-500">Nenhuma cultura cadastrada no sistema.</span> : null}</label>; })}<div className="grid gap-3 md:col-span-2 sm:flex sm:justify-end"><button disabled={saving} className="inline-flex min-w-44 items-center justify-center rounded-full bg-leaf-600 px-6 py-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60">{saving ? "Salvando..." : "Salvar cadastro"}</button></div></form></div><div className="mt-6 space-y-3">{records.length === 0 ? <p className="rounded-2xl border border-leaf-100 bg-white p-4 text-sm text-slate-600">Nenhum registro cadastrado.</p> : records.map((r) => <article key={r.id} className="rounded-2xl border border-leaf-100 bg-white p-4 shadow-soft"><strong className="text-[#123F2A]">{r.title}</strong><div className="mt-3 grid gap-2 sm:flex sm:flex-wrap"><button className="rounded-full border border-leaf-200 px-3 py-1 text-xs font-semibold text-leaf-700" onClick={() => setForm({ id: r.id, property_id: r.property_id ?? "", ...Object.fromEntries(Object.entries(r.data ?? {}).map(([k, v]) => [k, String(v ?? "")])) })}>Editar</button><button className="rounded-full border border-red-200 px-3 py-1 text-xs font-semibold text-red-700" onClick={async () => { if (!confirm("Deseja excluir este registro?")) return; const token = getStoredSupabaseAccessToken(); const d = await fetch(modulo === "cadastro-propriedade" ? `/api/acompanhamento/properties/${r.id}` : `/api/acompanhamento/${modulo}/${r.id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } }); if (d.ok) { setMessage({ type: "success", text: "Cadastro excluído com sucesso." }); load(); } }}>Excluir</button></div></article>)}</div>{modulo === "inteligencia-artificial" ? <div className="mt-8 rounded-3xl border border-leaf-100 bg-white p-4 shadow-soft"><h2 className="font-semibold text-[#123F2A]">Chat de IA para acompanhamento</h2><div className="mt-3 max-h-80 space-y-2 overflow-auto">{chatMessages.length===0?<p className="text-sm text-slate-500">Pergunte sobre manejo, cálculos agronômicos ou dúvidas técnicas deste acompanhamento.</p>:chatMessages.map((m,i)=><p key={i} className={`rounded-2xl p-3 text-sm ${m.role==="user"?"bg-leaf-50":"bg-slate-50"}`}>{m.content}</p>)}</div><form onSubmit={async (e)=>{e.preventDefault(); const q=chatText.trim(); if(!q||chatLoading) return; const token=getStoredSupabaseAccessToken(); if(!token) return; setChatMessages(c=>[...c,{role:"user",content:q}]); setChatText(""); setChatLoading(true); try{ const r=await fetch('/api/qa',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({question:`Contexto do módulo ${moduleData.title}. ${q}`})}); const p=await r.json(); if(!r.ok) throw new Error(p.error||'Falha ao consultar IA'); setChatMessages(c=>[...c,{role:"assistant",content:`${p.answer}

⚠️ Orientação técnica de apoio. Valide presencialmente antes de decisões críticas.`}]); }catch(err){ setChatMessages(c=>[...c,{role:"assistant",content:err instanceof Error?err.message:'Erro ao consultar IA'}]); } finally{ setChatLoading(false);} }} className="mt-3 grid gap-2 sm:flex"><input className="min-w-0 flex-1 rounded-2xl border border-leaf-100 px-4 py-3 text-sm" value={chatText} onChange={(e)=>setChatText(e.target.value)} placeholder="Digite sua pergunta" /><button disabled={chatLoading||!chatText.trim()} className="rounded-full bg-leaf-700 px-5 py-3 text-sm font-semibold text-white disabled:opacity-60">{chatLoading?"Enviando...":"Enviar"}</button></form></div> : null}</section>;
}
