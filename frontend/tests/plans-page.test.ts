/**
 * Apresentacao de preços na página publica (item 7): nunca criar preço ficticio.
 */
...
test("plano mensal exibe o preço formatado em BRL", () => {
  const formatted = formatPlanPrice(plan({ price_cents: 9700, price_period: "mês" }));
  assert.ok(formatted.includes("97,00"));
  assert.ok(formatted.endsWith("/mês"));
});

test("Presencial & Projetos Especiais exibe 'Sob consulta', nunca um preço", () => {
...
test("plano sob consulta sem texto configurado ainda assim não mostra R$ 0,00", () => {
